...
from invent._compat import figure, img
...
class Avatar(Widget):
    ...
    def render(self):
        ...
        fig = figure(avatar)
        fig.classList.add("invent-avatar")
        fig.classList.add(f"invent-avatar--{self.shape.lower()}")
        fig.classList.add(f"invent-avatar--{self.size.lower()}")
    ...
    def on_image_changed(self):
        ...
        """
        self.element.querySelector("img").src = self.image if self.image else ""

    def on_name_changed(self):
        """
        Update the alt text and hover tooltip on the <img> element.
        """
        img = self.element.querySelector("img")
    ...
    def on_shape_changed(self):
        ...
        for shape in ("circle", "rounded", "square"):
            self.element.classList.remove(f"invent-avatar--{shape}")
        self.element.classList.add(f"invent-avatar--{self.shape.lower()}")
    ...
    def on_size_changed(self):
        ...
        for size in (s.lower() for s in TSHIRT_SIZES if s is not None):
            self.element.classList.remove(f"invent-avatar--{size}")
        self.element.classList.add(f"invent-avatar--{self.size.lower()}")
